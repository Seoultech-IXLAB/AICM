...
def generate_images(model_name, prompts_path, save_path , safe_prompt, method="attention", lamda = "0.3", device='cuda:0', guidance_scale = 7.5, image_size=512, ddim_steps=50, num_samples=10, from_case=0 ,safe_step = 30, learning_late = 0.01,test=False):
    '''
    ...
    for _, row in df.iterrows():
        ...
        for num, im in enumerate(pil_images):
            im.save(f"{folder_path}/{model_name}_{method}_{case_number}_{num}.png")
        if test:
            break
    print("end")
...
if __name__=='__main__':
    ...
    test = args.test
    save_path = f"{args.save_path}_{safe_prompt}_{args.safe_steps}_{args.lamda}"
    print(f"save path : {save_path}")
    
    generate_images(model_name, prompts_path, save_path,safe_prompt, method=method, lamda = lamda, device=device,
                    guidance_scale = guidance_scale, image_size=image_size, ddim_steps=ddim_steps, num_samples=num_samples,from_case=from_case,safe_step= safe_step, learning_late = learning_late, test=test)
